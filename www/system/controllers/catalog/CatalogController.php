...
class CatalogController extends Controller{
    ...
    public function ShowItems($category){
        ...
        $this->meta_description = isset($category['meta_description']) ? $category['meta_description'] : '';
        $this->meta_keywords = isset($category['meta_keywords']) ? $category['meta_keywords'] : $category['meta_keywords'];
        if ($this->meta_title == ''){
    ...
    public function ShowItem(){
        if ($item = $this->ModelCatalog->GetItem($this->query[0])){
            ...
            $this->meta_description = isset($item['meta_description']) ? $item['meta_description'] : '';
            $this->meta_keywords = $item['meta_keywords'];
            if ($this->meta_title == ''){
                $this->meta_title = $this->page_title . '. ' .  $this->config->SiteTitle;
            }
            if (trim($this->meta_description == '')){
                $this->meta_description = mb_substr(trim(strip_tags($item['short_content'])),0,159);
            }
            if (trim($this->meta_description == '')){
                $this->meta_description = mb_substr(trim(strip_tags($item['content'])),0,159);
            }
            if ($item['img_name']){
